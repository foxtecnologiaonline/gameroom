...
- **Hospedagem 100% Vercel (27/08/2026) → fila de jobs trocada de BullMQ+Redis para Upstash QStash**: a Vercel é serverless (sem processo persistente para um worker BullMQ). Todo job assíncrono das seções §4/§7 abaixo (reabastecimento, expiração de reserva, emissão/entrega, devolução/estorno, nota fiscal) passou a ser publicado como mensagem HTTP no QStash, que chama de volta uma rota do próprio backend (`backend/src/jobs/http/`) — mesmas garantias de retry/backoff/DLQ e delayed job, sem Redis. O scheduler periódico de reabastecimento (antes um cron interno do BullMQ) virou uma QStash Schedule provisionada por fora do deploy. As referências a "BullMQ"/"Redis" no restante deste documento (§2, §7, §10, §11) descrevem a decisão original — o código e o `DEPLOY.md` são a fonte da verdade para a implementação atual.
